...
void PIStageProducer::DoConfigure(){
  ...
  if(m_connected_Y)     if(!m_controller->move(m_axis_Y.c_str(),pY)) EUDAQ_THROW("Y Movement failed");
  if(m_connected_Rot)   if(!m_controller->move(m_axis_Rot.c_str(),pRot)) EUDAQ_THROW("Rotation Movement failed");
